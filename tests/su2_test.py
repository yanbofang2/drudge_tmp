...
def test_su2_without_symbolic_index(local_ctx):
    """Test SU2 lattice drudge without abstract symbolic lattice index."""

    dr = SU2LatticeDrudge(local_ctx)
    p = dr.names
    half = Rational(1, 2)
    half_i = half / I
...
def test_su2_on_1d_heisenberg_model(local_ctx):
...
def test_su2_with_deformed_commutation(local_ctx):
    ...
    dr = SU2LatticeDrudge(local_ctx, specials={
